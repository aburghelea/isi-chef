	...
	<table class="table table-bordered">
		<thead>
			<tr>
				...
				<g:sortableColumn property="startDate" title="${message(code: 'rezervare.startDate.label', default: 'Start Date')}" />
			
				<g:sortableColumn property="endDate" title="${message(code: 'rezervare.endDate.label', default: 'End Date')}" />
			
				<th><g:message code="rezervare.masa.label" default="Masa" /></th>
			
				<g:sortableColumn property="tableId" title="${message(code: 'rezervare.tableId.label', default: 'Table Id')}" />
		...
		<g:each in="${rezervareInstanceList}" status="i" var="rezervareInstance">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td><g:link action="show" id="${rezervareInstance.id}">${fieldValue(bean: rezervareInstance, field: "startDate")}</g:link></td>
