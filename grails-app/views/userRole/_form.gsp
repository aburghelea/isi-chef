<%@ page import="ro.isi.auth.UserRole" %>
			...
			<div class="control-group fieldcontain ${hasErrors(bean: userRoleInstance, field: 'role', 'error')} required">
				...
				<div class="controls">
					<g:select id="role" name="role.id" from="${ro.isi.auth.Role.list()}" optionKey="id" required="" value="${userRoleInstance?.role?.id}" class="many-to-one"/>
			...
			<div class="control-group fieldcontain ${hasErrors(bean: userRoleInstance, field: 'user', 'error')} required">
				...
				<div class="controls">
					<g:select id="user" name="user.id" from="${ro.isi.auth.User.list()}" optionKey="id" required="" value="${userRoleInstance?.user?.id}" class="many-to-one"/>
