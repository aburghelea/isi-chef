	...
	<table class="table table-bordered">
		<thead>
			<tr>
				...
				<g:sortableColumn property="description" title="${message(code: 'produs.description.label', default: 'Description')}" />
			
				<g:sortableColumn property="preparationTime" title="${message(code: 'produs.preparationTime.label', default: 'Preparation Time')}" />
		...
		<g:each in="${produsInstanceList}" status="i" var="produsInstance">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td><g:link action="show" id="${produsInstance.id}">${fieldValue(bean: produsInstance, field: "name")}</g:link></td>
